        ...
        <tbody class="capitalize">
          @foreach ($ships as $k)
            <tr class="hover:bg-slate-200 hover:text-slate-500">
              ...
              <td>{{ $k->nama_kapal }}</td>
              <td>{{ $k->keperluan->muat_barang }}</td>
              <td>{{ $k->keperluan->bongkar }}</td>
              <td>{{ $k->keperluan->jenis_barang }}</td>
              <td>{{ $k->keperluan->keterangan }}</td>
              <td class="flex justify-center gap-2">
                <a href="{{ route('details.edit', [$k->keperluan->id]) }}" class="hover:text-yellow-600">
