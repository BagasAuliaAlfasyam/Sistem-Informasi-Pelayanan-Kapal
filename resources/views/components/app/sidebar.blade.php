    ...
    <div class="space-y-8">
      ...
      <div>
        ...
        <ul class="mt-3">
          ...
          <!-- Kapal -->
          <li
            class="px-3 py-2 cursor-pointer rounded-sm mb-0.5 hover:bg-slate-600 last:mb-0 @if (Request::is('dashboard/kapal*')) @elseif (Request::is('dashboard/details*')) {{ 'bg-slate-900' }} @endif"
            ...
            <div
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  <svg class="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                    <path class="fill-current @if (Request::is('kapal')) {{ 'text-indigo-300' }}@else{{ 'text-slate-400' }} @endif"
                      d="M13 15l11-7L11.504.136a1 1 0 00-1.019.007L0 7l13 8z" />
                    <path class="fill-current @if (Request::is('kapal')) {{ 'text-indigo-600' }}@else{{ 'text-slate-700' }} @endif"
                      d="M13 15L0 7v9c0 .355.189.685.496.864L13 24v-9z" />
                    <path class="fill-current @if (Request::is('kapal')) {{ 'text-indigo-500' }}@else{{ 'text-slate-600' }} @endif"
                ...
                <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                  <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-slate-400 @if (in_array(Request::segment(3), ['rincian'])) {{ 'rotate-180' }} @endif"
          ...
          <!-- Penjadwalan -->
          <li
            class="px-3 py-2 cursor-pointer rounded-sm mb-0.5 hover:bg-slate-600 last:mb-0 @if (Request::is('dashboard/penjadwalan')) {{ 'bg-slate-900' }} @endif"
            x-data="{ open: {{ Request::is('dashboard/penjadwalan') ? 1 : 0 }} }">
            <div
              class="block text-slate-200 hover:text-white truncate transition duration-150 @if (Request::is('penjadwalan.index')) {{ 'hover:text-white' }} @endif"
              @click.prevent="sidebarExpanded ? open = !open : sidebarExpanded = true">
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  <svg class="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                    <path class="fill-current @if (Request::is('penjadwalan')) {{ 'text-indigo-500' }}@else{{ 'text-slate-600' }} @endif"
                      d="M8 1v2H3v19h18V3h-5V1h7v23H1V1z" />
                    <path class="fill-current @if (Request::is('penjadwalan')) {{ 'text-indigo-500' }}@else{{ 'text-slate-600' }} @endif"
                      d="M1 1h22v23H1z" />
                    <path class="fill-current @if (Request::is('penjadwalan')) {{ 'text-indigo-300' }}@else{{ 'text-slate-400' }} @endif"
            ...
            <div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
              <ul class="pl-9 mt-1 @if (!Request::is('dashboard/penjadwalan')) {{ 'hidden' }} @endif" :class="open ? '!block' : 'hidden'">
                <li class="mb-1 last:mb-0">
                  <a class="block text-slate-400 hover:text-white transition duration-150 truncate @if (Route::is('penjadwalan.index')) {{ '!text-indigo-500' }} @endif"
                ...
                <li class="mb-1 last:mb-0">
                  <a class="block text-slate-400 hover:text-white transition duration-150 truncate @if (Route::is('tasks-list')) {{ '!text-indigo-500' }} @endif"
